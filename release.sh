#!/bin/bash
# This script automates all the tasks needed to make a new Hevo Android SDK release.
#
...
sed -i.bak "s,^\(##### _\).*\(_ - \[v\).*\(](https://github.com/hevo/hevo-android/releases/tag/v\).*\()\),\1$newDate\2$releaseVersion\3$releaseVersion\4,w changes.txt" README.md
if [ ! -s changes.txt ]; then
    printf "\n${RED}Err... README.md was not updated. The following command was used:\n"
    printf "sed -i.bak 's,^\(##### _\).*\(_ - \[v\).*\(](https://github.com/hevo/hevo-android/releases/tag/v\).*\()\),\1$newDate\2$releaseVersion\3$releaseVersion\4,' README.md${NC}\n\n"
    abort
fi
...
printf "Make sure you make a new release at https://github.com/hevo/hevo-android/releases/new\n"
printf "Also, do not forget to update our CHANGELOG (https://github.com/hevo/hevo-android/wiki/Changelog)\n"
