package com.hevodata.example.hello;
...
/**
 * A little application that allows people to update their Hevo information,
 * and receive push notifications from a Hevo project.
 *
 * For more information about integrating Hevo with your Android application,
 * please check out:
 *
 *     https://hevo.com/docs/integration-libraries/android
 *
 * For instructions on enabling push notifications from Hevo, please see
 *
 *     https://hevo.com/docs/people-analytics/android-push
 *
 * @author hevo
 *
 */
public class MainActivity extends Activity {
    ...
    /*
     * You will use a Hevo API token to allow your app to send data to Hevo. To get your token
     * - Log in to Hevo, and select the project you want to use for this application
     ...
     */
    public static final String HEVO_API_TOKEN = "NOT A REAL TOKEN";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // Initialize the Hevo library for tracking and push notifications.
        mHevo = HevoAPI.getInstance(this, HEVO_API_TOKEN);


        // We also identify the current user with a distinct ID, and
        // register ourselves for push notifications from Hevo.

        mHevo.identify(trackingDistinctId); //this is the distinct_id value that
        // will be sent with events. If you choose not to set this,
        // the SDK will generate one for you

        mHevo.getPeople().identify(trackingDistinctId); //this is the distinct_id
        ...
        mHevo.getPeople().initPushHandling(ANDROID_PUSH_SENDER_ID);
    ...
    protected void onResume() {
        ...
        try {
            ...
            mHevo.registerSuperPropertiesOnce(properties);
        } catch (final JSONException e) {
            throw new RuntimeException("Could not encode hour first viewed as JSON");
        }

        // Now we send an event to Hevo. We want to send a new
        ...
        try {
            ...
            mHevo.track("App Resumed", properties);
        } catch(final JSONException e) {
            throw new RuntimeException("Could not encode hour of the day in JSON");
        }

        // If you have notifications and you have set AutoShowHevoUpdates set to false,
        ...
        mHevo.getPeople().showNotificationIfAvailable(this);
        OnHevoUpdatesReceivedListener listener = new OnHevoUpdatesReceivedListener() {
            @Override
            public void onHevoUpdatesReceived() {
                mHevo.getPeople().joinExperimentIfAvailable();
            }
        };
        mHevo.getPeople().addOnHevoUpdatesReceivedListener(listener);
    }

    // Associated with the "Send to Hevo" button in activity_main.xml
    // In this method, we update a Hevo people profile using HevoAPI.People.set()
    // and set some persistent properties that will be sent with
    // all future track() calls using HevoAPI.registerSuperProperties()
    public void sendToHevo(final View view) {
        ...
        final HevoAPI.People people = mHevo.getPeople();
        ...
        // Hevo events are separate from Hevo people records,
        ...
        try {
            ...
            mHevo.registerSuperProperties(domainProperty);
        } catch (final JSONException e) {
            throw new RuntimeException("Cannot write user email address domain as a super property");
        }

        // In addition to viewing the updated record in hevo's UI, it might
        ...
        // before the event is dispatched to Hevo)
        mHevo.track("update info button clicked", null);
    }

    // This is an example of how you can use Hevo's revenue tracking features from Android.
    ...
    protected void onDestroy() {
        ...
        // To preserve battery life, the Hevo library will store
    ...
    private String getTrackingDistinctId() {
        ...
        String ret = prefs.getString(HEVO_DISTINCT_ID_NAME, null);
        if (ret == null) {
            ret = generateDistinctId();
            final SharedPreferences.Editor prefsEditor = prefs.edit();
            prefsEditor.putString(HEVO_DISTINCT_ID_NAME, ret);
            prefsEditor.commit();
        }
    ...
    // in your users table to store hevo distinct_id, so it is easily
    ...
    private HevoAPI mHevo;
    private static final String HEVO_DISTINCT_ID_NAME = "Hevo Example $distinctid";
    private static final int PHOTO_WAS_PICKED = 2;
    private static final String LOGTAG = "Hevo Example Application";
}
